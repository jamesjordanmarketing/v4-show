...
- 📄 Path to the new task file: {{NEW_TEST_CARRY_PROMPT_PATH}}
- 📄 Path to the new conductor file: {{NEW_TEST_CONDUCTOR_PATH}}

### Step 3: Run the New Context Prompt

Execute the new context prompt in {{NEW_TEST_CARRY_PROMPT_PATH}}
...
{{NEW_TEST_CARRY_CONTEXT_PATH}} and {{NEW_TEST_ENHANCED_PLAN_PATH}}
...
Execute the test conductor prompt in {{NEW_TEST_CONDUCTOR_PATH}} to customize the new test conductor prompt. 
Wait for 20 seconds to make sure the prompt has finished executing. 

Check that the output in {{NEW_TEST_CONDUCTOR_OUTPUT_PATH}}  
...
## Next Steps
The human will copy the {{NEW_TEST_CONDUCTOR_OUTPUT_PATH}} prompt into the next ai testing agent panel.
