...
Put the optimized version here: {{NEW_TEST_CONDUCTOR_OUTPUT_PATH}}
IMPORTANT in the output file change content `{{NEW_TEST_CONDUCTOR_OUTPUT_PATH}}` to be `pmc\core\active-task-unit-tests-2-enhanced.md`as that is where the customized test plan will be contained when we start the new task.