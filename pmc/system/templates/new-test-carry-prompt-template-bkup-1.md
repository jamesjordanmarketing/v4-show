...
Create an enhanced test plan at: `{{NEW_TEST_ENHANCED_PLAN_PATH}}`
...
**Completion Check**: Verify that `{{NEW_TEST_ENHANCED_PLAN_PATH}}` exists, is task-specific, and uses directive language throughout before proceeding to Step 3.

Once confirmed copy `{{NEW_TEST_ENHANCED_PLAN_PATH}}` into pmc\core\active-task-unit-tests-2-enhanced.md replacing everything already there.
