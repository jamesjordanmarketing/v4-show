...
```bash
node bin\aplio-agent-cli.js start-task "{{CURRENT_TASK_ID}}"
...
Execute the new carry over context prompt {{NEW_PANEL_NEW_TASK_PATH}}
Wait for 60 seconds to make sure the prompt has finished executing. 
You must check that the information in: {{NEW_TASK_CARRY_CONTEXT_PATH}}
...
Execute the new conductor prompt in {{NEW_TASK_CONDUCTOR_PATH}}
Wait for 60 seconds to make sure the prompt has finished executing. 

Check that the output in {{NEW_TASK_CONDUCTOR_OUTPUT}} 
has been updated with content relevant to this task and test.
...
The human will copy the {{NEW_TASK_CONDUCTOR_OUTPUT}} into the next ai testing agent panel.