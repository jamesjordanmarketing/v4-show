...
Create a detailed context carryover file at: `{{NEW_TEST_CARRY_CONTEXT_PATH}}`
...
One more reminder. Your job is ONLY to update {{NEW_TEST_CARRY_CONTEXT_PATH}} with an in depth context build of all of the information it will need to fully create a customized test plan. Remove any sections not used.

**Completion Check**: Verify that `{{NEW_TEST_CARRY_CONTEXT_PATH}}` has been updated with and contains all required sections before proceeding to Step 2.
...
- `{{NEW_TEST_CARRY_CONTEXT_PATH}}` (context from Step 1)
...
Create the first enhanced test plan at: `{{NEW_TEST_ENHANCED_PLAN_PATH}}-1-and-2`
...
Create the second enhanced test plan at: `{{NEW_TEST_ENHANCED_PLAN_PATH}}-3-thru-5`
...
**Completion Checks**: 
1. Verify that `{{NEW_TEST_ENHANCED_PLAN_PATH}}-1-and-2` exists and contains:
   - Complete Phase 0, 1, and 2 instructions
   - Proper test-approach methodology
   - Completion report section for handoff
   
2. Verify that `{{NEW_TEST_ENHANCED_PLAN_PATH}}-3-thru-5` exists and contains:
...
1. Copy `{{NEW_TEST_ENHANCED_PLAN_PATH}}-1-and-2` into `pmc\core\active-task-unit-tests-2-enhanced-phases-1-2.md` replacing everything already there
2. Copy `{{NEW_TEST_ENHANCED_PLAN_PATH}}-3-thru-5` into `pmc\core\active-task-unit-tests-2-enhanced-phases-3-5.md` replacing everything already there
