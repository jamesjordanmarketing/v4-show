...
1.  **Generate Task Approach:**
    ...
    *   Read the file {{NEW_TASK_CARRY_CONTEXT_PATH}} for relevant context from the task test finished just before this one.
...
Based on the current pmc\core\active-task.md file and the carry over context in {{NEW_TASK_CARRY_CONTEXT_PATH}}
Is this task implementation conductor prompt enough? too much? please optimize it for the next ai implementation agent and put it in the output file: {{NEW_TASK_CONDUCTOR_OUTPUT}}


Based on the current pmc\core\active-task.md file and the carry over context in {{NEW_TASK_CARRY_CONTEXT_PATH}}
Is this enough? too much? please optimize it for the next ai implementation agent who must implement {{CURRENT_TASK_ID}}
