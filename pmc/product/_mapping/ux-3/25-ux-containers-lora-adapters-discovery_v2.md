...
## Discovery Document v2

**Date:** 2026-03-03  
**Scope:** Issues A, B, C, D from investigation prompt — adapter deployment confirmation, subsequent run behavior, adapter status ping tool, and endpoint restart tool  
**Output:** Technical requirements for a new **Adapter Detail Page** linked from each Adapter History entry on `/workbase/[id]/fine-tuning/launch`
...
5. [Issue D — Endpoint Restart Tool](#4b-issue-d--endpoint-restart-tool)
6. [Unified Solution: Adapter Detail Page](#6-unified-solution-adapter-detail-page)
7. [Barrier Analysis](#7-barrier-analysis)
...
### 2.3 Technicals: Deployment Report
...
On the new Adapter Detail Page (see [Section 6](#6-unified-solution-adapter-detail-page)):
...
### 3.5 Technical Requirements: Adapter Lifecycle Management
...
### 4.3 Technical Requirements: Adapter Status Ping Tool
...
## 4b. Issue D — Endpoint Restart Tool

### 4b.1 Core Requirements

> A tool on the Adapter Detail Page that:
> a. Shows the results of the last worker restart triggered by this Workbase
> b. Confirms whether the restart successfully picked up the new adapter
> c. Provides a button to manually trigger a worker restart
> d. Reports success/failure and confirms the correct adapter is active

### 4b.2 Data Verification — Current Restart Behavior

**How worker restarts currently work** (from `refresh-inference-workers.ts`):

The `refreshInferenceWorkers` Inngest function performs a 6-step worker cycle:

| Step | Name | Action | Duration |
|------|------|--------|----------|
| 1 | `scale-down-workers` | Sets `workersMin=0` via RunPod GraphQL `saveEndpoint` | Instant |
| 2 | `wait-for-workers-terminated` | Polls `INFERENCE_API_URL/health` every 5s until total workers = 0 | Up to 90s |
| 3 | `scale-up-workers` | Restores `workersMin/workersMax`, ensures `MAX_LORAS=5` | Instant |
| 4 | `wait-for-workers-ready` | Polls `/health` every 5s until `ready > 0` or `idle > 0` | Up to 180s |
| 5 | `verify-adapter-available` | Sends test inference (`/runsync`) with `model: adapterName`, checks for `COMPLETED` | ~2-10s |
| 6 | `mark-endpoints-ready` | Updates `pipeline_inference_endpoints` to `status='ready'`, sets `ready_at` | Instant |

**What IS currently stored about restarts:**

| Data Point | Location | How Set |
|------------|----------|---------|
| Endpoint `status` | `pipeline_inference_endpoints.status` | Step 6 sets `ready` on success |
| Endpoint `ready_at` | `pipeline_inference_endpoints.ready_at` | Step 6 sets timestamp |
| Inngest step outputs | Inngest dashboard only | Each step returns data but it's NOT persisted to DB |

**What is NOT stored (gaps):**

| Missing Data Point | Available At | Currently |
|--------------------|-------------|-----------|
| Restart initiation timestamp | Step 1 | Not stored |
| Scale-down duration | Step 2 return `waitedMs` | Inngest step output only |
| Scale-up duration | Step 4 return `waitedMs` | Inngest step output only |
| Adapter verification result | Step 5 return `verified: true/false` | Inngest step output only |
| Which adapters were loaded after restart | Step 5 inference test | Not checked per-adapter |
| Restart trigger source (auto-deploy vs manual) | Nowhere | Not tracked |
| Restart success/failure history | Inngest logs | Not queryable from app |

### 4b.3 Technical Requirements: Endpoint Restart Tool

#### A. New Database Table — `endpoint_restart_log`

```sql
CREATE TABLE endpoint_restart_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  workbase_id UUID NOT NULL REFERENCES workbases(id),
  job_id UUID NOT NULL REFERENCES pipeline_training_jobs(id),
  adapter_name TEXT NOT NULL,                      -- "adapter-e8fa481f"
  runpod_endpoint_id TEXT NOT NULL,                -- "780tauhj7c126b"
  trigger_source TEXT NOT NULL DEFAULT 'auto',     -- 'auto' | 'manual'
  
  -- Phase tracking
  status TEXT NOT NULL DEFAULT 'initiated'
    CHECK (status IN ('initiated', 'scaling_down', 'workers_terminated',
...
CREATE INDEX idx_restart_log_workbase_latest
  ON endpoint_restart_log (workbase_id, initiated_at DESC);
CREATE INDEX idx_restart_log_job
  ON endpoint_restart_log (job_id);
```

#### B. API Route — `POST /api/pipeline/adapters/[jobId]/restart`

**Purpose:** Trigger a manual worker restart for a specific adapter/workbase.
...
```
1. Auth check — user must own the workbase
2. Validate job exists and has hf_adapter_path (has been deployed)
3. Derive adapterName from jobId
4. Create endpoint_restart_log row with status='initiated', trigger_source='manual'
5. Fire Inngest event 'pipeline/endpoint.restart.requested'
6. Return restartLogId immediately (frontend polls for progress)
```

The restart is a **long-running operation** (up to ~4.5 minutes: 90s shutdown + 180s startup + verification). The API returns immediately with a `restartLogId`, and a new Inngest function executes the actual restart asynchronously.

#### C. API Route — `GET /api/pipeline/adapters/[jobId]/restart-status`

**Purpose:** Read the latest restart log for this job (or a specific logId).
...
interface RestartStatusResponse {
  ...
  data?: {
    ...
    workerStateAfter: { ready: number; idle: number; running: number; initializing: number } | null;
...
#### D. Inngest Function — `restartInferenceWorkers`

A new Inngest function triggered by `pipeline/endpoint.restart.requested`:

```typescript
// Event: 'pipeline/endpoint.restart.requested'
// Data: { jobId, workbaseId, adapterName, restartLogId, endpointId }
```

**Execution steps (updates `endpoint_restart_log` at each phase):**

```
Step 1: Read current endpoint config via GraphQL
  → Check if adapter is in LORA_MODULES BEFORE restart
  → Record adapter_in_lora_modules, lora_modules_snapshot
  → Update log: status='scaling_down', scale_down_at

Step 2: Scale down workers (workersMin=0) via GraphQL saveEndpoint
  → Update log: scale_down_success

Step 3: Poll /health until workers = 0 (timeout 90s)
  → Update log: status='workers_terminated', workers_terminated_at

Step 4: Scale up workers (restore workersMin/workersMax, MAX_LORAS=5)
  → Update log: status='scaling_up', scale_up_at, scale_up_success

Step 5: Poll /health until ready > 0 (timeout 180s)
  → Update log: status='workers_ready', workers_ready_at, worker_state_after

Step 6: Verify adapter via inference ping (POST /runsync with model=adapterName)
  → Update log: status='verifying', verification_at, adapter_verified

Step 7: Final status
  → Calculate total_duration_ms
  → Update log: status='completed' or 'failed', completed_at
```

**Key difference from existing `refreshInferenceWorkers`:**
- Writes progress to `endpoint_restart_log` at each step (existing function doesn't persist progress)
- Explicitly verifies the TARGET adapter (not just "any adapter works")
- Reads `LORA_MODULES` after restart to confirm the adapter is listed
- Sets `adapter_verified` based on whether inference with `model: adapterName` succeeds

#### E. Frontend Component — `<EndpointRestartTool>`

Displays on the Adapter Detail Page in a dedicated card.

**State 1: No previous restart**
```
┌─────────────────────────────────────────────────┐
│ 🔄 Endpoint Restart                             │
│                                                 │
│ No restart history for this adapter.            │
│                                                 │
│ If the adapter was deployed but workers haven't │
│ picked it up, use the button below to cycle     │
│ the inference workers.                          │
│                                                 │
│ [🔄 Restart Endpoint Workers]                   │
└─────────────────────────────────────────────────┘
```

**State 2: Last restart succeeded — adapter confirmed active**
```
┌─────────────────────────────────────────────────┐
│ 🔄 Endpoint Restart                             │
│                                                 │
│ Last Restart: Mar 3, 2026 1:30 PM (auto)        │
│ Duration: 48s                                   │
│                                                 │
│ ✅ Workers scaled down successfully             │
│ ✅ Workers restarted and ready (1 ready)        │
│ ✅ Adapter adapter-e8fa481f verified active     │
│ ✅ Adapter confirmed in LORA_MODULES (3 total)  │
│                                                 │
│ [🔄 Restart Endpoint Workers]                   │
└─────────────────────────────────────────────────┘
```

**State 3: Last restart succeeded — but adapter NOT verified**
```
┌─────────────────────────────────────────────────┐
│ 🔄 Endpoint Restart                             │
│                                                 │
│ Last Restart: Mar 3, 2026 1:30 PM (manual)      │
│ Duration: 52s                                   │
│                                                 │
│ ✅ Workers scaled down successfully             │
│ ✅ Workers restarted and ready (1 ready)        │
│ ⚠️ Adapter adapter-e8fa481f NOT verified        │
│    Inference returned error: "Model not found"  │
│ ❌ Adapter NOT in LORA_MODULES                  │
│    Current adapters: [adapter-6fd5ac79]         │
│                                                 │
│ ⚠️ The adapter may not have been deployed to    │
│ RunPod before this restart. Check the           │
│ Deployment Report section above.                │
│                                                 │
│ [🔄 Restart Endpoint Workers]                   │
└─────────────────────────────────────────────────┘
```

**State 4: Restart in progress**
```
┌─────────────────────────────────────────────────┐
│ 🔄 Endpoint Restart                             │
│                                                 │
│ ⏳ Restart in progress...                       │
│                                                 │
│ ✅ Workers scaling down...         (2s ago)     │
│ ✅ Workers terminated              (15s ago)    │
│ ⏳ Waiting for workers to start... (polling)    │
│ ○  Verify adapter                               │
│                                                 │
│ [Cancel] (disabled during restart)              │
└─────────────────────────────────────────────────┘
```

**State 5: Last restart failed**
```
┌─────────────────────────────────────────────────┐
│ 🔄 Endpoint Restart                             │
│                                                 │
...
│                                                 │
│ ✅ Workers scaled down successfully             │
│ ❌ FAILED at: "Waiting for workers to start"    │
│    Error: Workers did not reach ready state      │
│    within 180s                                  │
│                                                 │
│ ⚠️ Workers may still be starting. Wait a few    │
│ minutes and check the Adapter Status Ping above.│
│                                                 │
│ [🔄 Retry Restart]                              │
└─────────────────────────────────────────────────┘
```

#### F. Integration with Auto-Deploy Pipeline

The existing `refreshInferenceWorkers` Inngest function (triggered automatically after `autoDeployAdapter`) should ALSO write to `endpoint_restart_log` so that the last restart result is always visible regardless of whether it was triggered manually or automatically.

**Change to `refresh-inference-workers.ts`:**
- At Step 1: Create an `endpoint_restart_log` row with `trigger_source='auto'`
- At each subsequent step: Update the log row with timing and results
- At Step 6: Update `completed_at`, `adapter_verified`, `lora_modules_snapshot`

This ensures the "Last Restart" display always shows the most recent restart, whether auto or manual.

### 4b.4 Decision Logic — "Did the restart work correctly?"

The tool must answer three compound questions:

| Question | How Verified | Display |
|----------|-------------|---------|
| **1. Did the API call successfully restart the endpoint?** | `scale_down_success=true` AND `scale_up_success=true` AND `status='completed'` | ✅/❌ with error details |
| **2. Was the timing correct — was the adapter deployed BEFORE the restart?** | `adapter_in_lora_modules=true` at Step 1 (pre-restart check). If `false`, the adapter wasn't in `LORA_MODULES` when the restart began, meaning the deploy step didn't complete before the restart fired | ✅/⚠️ with explanation |
| **3. After restart, is the new adapter active and confirmed?** | `adapter_verified=true` (inference ping returned `COMPLETED` with `model: adapterName`) | ✅/❌ with inference error |

**Combined verdict logic:**

```typescript
function getRestartVerdict(log: EndpointRestartLog): RestartVerdict {
  if (log.status === 'failed') {
    return { verdict: 'FAILED', reason: `Restart failed at step: ${log.errorStep}`, severity: 'error' };
  }
  ...
  if (log.adapterVerified) {
    return { verdict: 'SUCCESS', reason: 'Adapter is active and verified on inference endpoint', severity: 'success' };
  }
...
| Barrier | Severity | Mitigation |
|---------|----------|------------|
| **Long-running operation** — Worker restart takes 45s-270s (scale down + scale up). Cannot be done synchronously in a single API call | High | Use Inngest async function with `endpoint_restart_log` row for progress tracking; frontend polls every 5s |
| **Shared endpoint across workbases** — Restarting workers affects ALL workbases, not just the one triggering the restart. Other workbases' adapters may be temporarily unavailable during the ~45s restart window | High | Display clear warning: "This will briefly interrupt ALL adapter inference across all Work Bases." Add a confirmation dialog before manual restart |
| **Race condition with auto-deploy** — If the user clicks "Restart" while `autoDeployAdapter` is already running `refreshInferenceWorkers`, two concurrent restarts could conflict | Medium | Check `endpoint_restart_log` for any in-progress restart before allowing a new one. Return error: "A restart is already in progress" |
| **Inngest concurrency limit** — `refreshInferenceWorkers` has `concurrency: { limit: 1 }`. A manual restart must respect this to avoid conflicting with auto-restarts | Medium | Route manual restarts through the same Inngest function (or a sibling with `concurrency: { limit: 1, key: endpointId }`) |
| **Cost of unnecessary restarts** — Each restart triggers a cold-start billing event on RunPod ($0.50-2.00 per cycle depending on GPU) | Medium | Show cost estimate in confirmation dialog. Add cooldown: "Last restart was X minutes ago — are you sure?" |
| **Adapter timing verification** — The tool needs to confirm the adapter was in `LORA_MODULES` BEFORE the restart to verify correct timing. This requires reading the endpoint config at the START of the restart, not just at the end | Low | Step 1 of restart reads `LORA_MODULES` and records `adapter_in_lora_modules` before any scaling changes |
| **Workers already at 0** — If the endpoint is already scaled to 0 (idle timeout), the "scale down" step is a no-op and "scale up" triggers a normal cold start. The restart tool should handle this gracefully | Low | Check worker count at Step 1. If already 0, skip scale-down and proceed directly to scale-up |

---

## 6. Unified Solution: Adapter Detail Page
...
### 6.2 Navigation — Link from Adapter History
...
### 6.3 Adapter Detail Page Layout
...
│ │                     │ │ 🔄 Endpoint Restart       │   │
│ │ ✅ Verified         │ │                           │   │
│ │    Inference OK     │ │ Last: Mar 3 1:30 PM (auto)│   │
│ └─────────────────────┘ │ Duration: 48s             │   │
│                         │                           │   │
│                         │ ✅ Scale down OK          │   │
│                         │ ✅ Workers ready (1)      │   │
...
### 6.4 Data Sources for Each Section
...
| Endpoint Restart | `endpoint_restart_log` table (new) + live RunPod restart | `GET /api/pipeline/adapters/[jobId]/restart-status` (new) + `POST /api/pipeline/adapters/[jobId]/restart` (new) |
...
| `src/inngest/functions/restart-inference-workers.ts` | Inngest Function | Manual restart with `endpoint_restart_log` tracking |
...
| `src/components/pipeline/EndpointRestartTool.tsx` | Component | Endpoint restart widget with progress states |
| `src/components/pipeline/DeploymentTimeline.tsx` | Component | Deployment report visualization |

### 6.6 Database Changes Required
...
| New table | `endpoint_restart_log` | Full schema in [Section 4b.3.A](#a-new-database-table--endpoint_restart_log) |

---

## 7. Barrier Analysis — Cross-Cutting Concerns

### 7.1 Inngest Observability

The biggest structural barrier across all four issues is that **Inngest step outputs are ephemeral**. The data needed for the deployment report (RunPod responses, worker refresh timing, verification results) and the restart history currently exists only as Inngest step return values and console logs.

**Resolution:** Modify the Inngest functions to persist key data:
- `auto-deploy-adapter.ts`: Write `deployment_log` JSONB to `pipeline_training_jobs` (2 code changes)
- `refresh-inference-workers.ts`: Write restart results to `endpoint_restart_log` table (6 code changes — one per step)
- New `restart-inference-workers.ts`: Shares the same write-to-`endpoint_restart_log` pattern but triggered manually
- 2 SAOL migrations for new columns + 1 for new table

### 7.2 Single Shared Endpoint
...
- **Manual restarts affect all workbases** — this must be clearly communicated in the UI

**Resolution:** The adapter removal logic must query DB for all `active` adapters across ALL workbases and only remove the specific adapter being superseded, never touching adapters belonging to other workbases. The restart tool must display a warning about cross-workbase impact.
...
| Worker cycling (adapter deploy or manual restart) | ~$0.50-2.00 per cycle (cold starts) |
...
|-------|------|-------------|
| **Phase 1** | DB migration (`deployment_log`, `adapter_status` columns, `endpoint_restart_log` table) | SAOL |
| **Phase 2** | Modify `autoDeployAdapter` to capture and persist deployment data | Phase 1 |
| **Phase 3** | Modify `refreshInferenceWorkers` to write to `endpoint_restart_log` | Phase 1 |
| **Phase 4** | Build Adapter Detail Page with Deployment Report | Phase 2 |
| **Phase 5** | Build Adapter Status Ping API and component | Phase 1 |
| **Phase 6** | Build Endpoint Restart API, Inngest function, and component | Phase 1, Phase 3 |
| **Phase 7** | Implement adapter lifecycle management (supersede old adapters, remove from RunPod) | Phase 2 |
| **Phase 8** | Link Adapter History entries to detail page | Phase 4 |
