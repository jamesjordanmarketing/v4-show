...
## E01 Build Artifacts (Already Completed)

These backend changes are in place from E01:

1. **DB columns on `training_sets`:**
...
3. **POST `/api/workbases/[id]/training-sets/[tsId]/bypass`** endpoint:
...
The current header button group (~lines 118-130):
```tsx
        <div className="flex gap-2">
          <Button size="sm" variant="outline"
            onClick={() => router.push(`/workbase/${workbaseId}/fine-tuning/conversations/batch`)}>
...
The current lucide-react import (~lines 16-18):
```typescript
import { 
  MoreVertical, Eye, Edit, Copy, Download, Trash2,
  ArrowUpDown, ArrowUp, ArrowDown, Check, X,
  FileJson, Plus, AlertTriangle, RotateCcw,
} from 'lucide-react';
```
...
Check whether `useRouter` is already imported from `next/navigation`. If it is NOT, add:
```typescript
import { useRouter } from 'next/navigation';
```

Inside the component function body, add (before the `useConversationStore` destructure):
```typescript
const router = useRouter();
```

**Important:** If `useRouter` is already imported and instantiated, skip this step.

### 3c. Update training sets query mapping to include new error fields

The current `useQuery` for `trainingFiles` (~lines 119-143) has this `.map()`:
```typescript
...
In the `trainingFiles.map()` block inside the "Add to Existing Training Set" dropdown (~lines 560-590), find the section that starts with:
```typescript
