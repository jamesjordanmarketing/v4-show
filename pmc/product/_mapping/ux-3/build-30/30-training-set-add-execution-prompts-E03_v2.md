...
**Change log (v2):** Updated all "current" code blocks to match the actual post-E02 codebase. Fixed ConversationTable line count (872, not 843). Fixed conversations/page.tsx line count (253, not 244). Fixed conversation-store.ts comment text for `selectAllConversations`. Fixed conversation-storage-service.ts filter formatting (multi-line braces, not compressed). Fixed conversations/route.ts service call formatting (one-per-line filter object). Updated ConversationTable imports context to include `useRouter` (added in E02). Confirmed E02 artifacts are live and verified.
...
**This is the LAST execution prompt in the Spec 30 sequence.** After E03, all training set monitoring, bypass/recovery, cross-page selection, enrichment sorting, enrichment filtering, and page sizing features are complete.
...
### Design Token Rules  
**MANDATORY:** Use `bg-background`, `bg-card`, `bg-muted`, `text-foreground`, `text-muted-foreground`, `border-border`. **NEVER** use `zinc-*`, `slate-*`, or hardcoded `gray-*`. For brand accent use `text-duck-blue`.
...
- "Training Sets" button in conversations page header (using `Layers` icon)
- ConversationTable dropdown handles `failed` training sets with navigation to monitoring page
- `ExternalLink` added to lucide-react imports in ConversationTable
- `useRouter` imported and initialized in ConversationTable (line 3: `import { useRouter } from 'next/navigation';`, line 101: `const router = useRouter();`)
- Training sets query mapping includes `lastBuildError` and `failedConversationIds`
...
### ConversationTable — Current Imports (Post-E02)
```typescript
'use client';
...
// ... other imports (Conversation types, hooks, stores, etc.)
```
(Note: `useRouter` import at line 3 and `ExternalLink` in lucide imports were both added in E02.)
...
### ConversationTable — Current Component Signature + Store Destructure
```typescript
...
```
(Note: `const router = useRouter();` was added in E02.)
...
Find the `<ConversationTable>` JSX:
```tsx
      <ConversationTable
...
Find the filters section in the JSX — it has `<Select>` dropdowns for status, tier, and quality_min. After the last existing filter (quality_min `</Select>` closing tag), add the enrichment status filter:
...
**Replace with (add `enrichment_status` filter before sorting):**
```typescript
    if (filters?.quality_min !== undefined) {
      query = query.gte('quality_score', filters.quality_min);
    }
...
**Replace with (add `enrichment_status` extraction):**
```typescript
    const quality_max = searchParams.get('quality_max')
      ? parseFloat(searchParams.get('quality_max')!)
      : undefined;
...
| `ConversationTable.tsx` | — | ✓ Failed dropdown + useRouter | ✓ Selection, sort props, label |
...
## Completeness Check (Spec 30 — All Prompts)

After E03 completes, the full Spec 30 feature set is delivered:

| Feature | Prompt | Status |
|---------|--------|--------|
| DB columns (`last_build_error`, `failed_conversation_ids`) | E01 | ✅ Done |
| Inngest error capture + success clearing | E01 | ✅ Done |
| GET API returns error fields | E01 | ✅ Done |
| Bypass API endpoint | E01 | ✅ Done |
| Training Sets monitoring page | E02 | ✅ Done |
| "Training Sets" nav button | E02 | ✅ Done |
| Failed training set in dropdown | E02 | ✅ Done |
| Cross-page selection | E03 | Pending |
| Page size selector | E03 | Pending |
| Server-side enrichment sort | E03 | Pending |
| Enrichment status filter | E03 | Pending |
| "Not Enriched" label fix | E03 | Pending |

No additional prompts needed. E03 is the final execution prompt for Spec 30.
