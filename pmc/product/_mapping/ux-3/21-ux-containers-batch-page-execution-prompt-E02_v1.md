...
**Prerequisite:** E01 must be fully complete (DB column added, 3 backend files updated)  
...
Before writing any code, verify that E01 was completed. The `GET /api/batch-jobs` route must now accept a `workbaseId` query param. Check this by reading:
- `src/app/api/batch-jobs/route.ts` — confirm it parses `searchParams.get('workbaseId')`
- `src/lib/services/batch-job-service.ts` — confirm `getAllJobs` accepts `workbaseId` filter
...
Read the file fully first. Find the `handleSubmit` function. Inside it, after a successful batch submission, there is currently this redirect:
...
Run TypeScript compilation to confirm zero errors:
...
**If there are errors:**
- `Cannot find module '@/components/ui/progress'` — this component should already exist. Check `src/components/ui/progress.tsx`. If missing, the legacy page uses it so it must exist.
...
Once all tasks pass and the smoke test checklist is complete:
...
git status
git commit -m "Add workbase-scoped batch job watcher (Spec 21)

- Add workbase_id column to batch_jobs with index and backfill
- Filter batch jobs by workbaseId in API and service layer
