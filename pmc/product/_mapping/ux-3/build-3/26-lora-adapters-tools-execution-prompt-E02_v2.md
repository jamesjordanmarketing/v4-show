...
1. **Platform Context env vars**: `HF_REPO_PREFIX` corrected to `HF_ADAPTER_REPO_PREFIX` — the actual env var key used in `auto-deploy-adapter.ts` (line 48: `process.env.HF_ADAPTER_REPO_PREFIX`).
2. **Change 3a**: Removed the conditional fallback guard for `deleteRepo`. The function IS exported from the installed `@huggingface/hub` package — confirmed in `node_modules/@huggingface/hub/dist/src/lib/delete-repo.d.ts`.
3. **Change 4b**: Added explicit "Find" block showing the exact `void (await step.run('scale-down-workers', ...))` pattern in the actual file (line 71). The `void` wrapper around the entire awaited expression must be removed when replacing.
4. **Change 4e**: Removed `const workersReadyResult =` from the replacement block — the result is never referenced downstream so this would generate a TypeScript "declared but not read" error. The step is now a plain `await step.run(...)`.
5. **TypeScript validation section**: Removed the incorrect `workersReadyResult` suppression note; updated to reflect the actual pre-existing error.
...
- `GPU_CLUSTER_API_KEY` or `RUNPOD_API_KEY` — for RunPod inference API (Bearer header); used in `refresh-inference-workers.ts` and `restart-inference-workers.ts`
- `INFERENCE_API_URL` — RunPod inference endpoint base URL
- `RUNPOD_INFERENCE_ENDPOINT_ID` — the shared endpoint ID
- `HF_TOKEN` — HuggingFace access token
- `HF_ADAPTER_REPO_PREFIX` — HuggingFace repo name prefix (env var key; local const is `HF_REPO_PREFIX`)
...
> Note: `deleteRepo` is confirmed exported from the installed version of `@huggingface/hub` (verified in `node_modules/@huggingface/hub/dist/src/lib/delete-repo.d.ts`). No fallback guard is needed.

#### Change 3b: Update `HFUploadResult` interface

Find the current interface (approximately lines 58–61):
```typescript
...
Find the section in Step 2+3 (upload to HuggingFace) that currently logs the commit and returns (approximately lines 276–281):
```typescript
        console.log(
...
In Step 4 (`update-runpod-lora-modules`), find the section labeled `// ---- 4b: Parse existing LORA_MODULES and add new adapter ----` (approximately lines 362–387). The current block is:
...
Find the final `return` statement at the end of Step 4 that currently reads (approximately line 471):
```typescript
...
> Note: `saveData` is already in scope at this point (declared a few lines earlier). `saveData.errors` was previously the throw condition — if execution reaches this return, `saveData.errors` is guaranteed falsy, so `runpodSaveSuccess` will always be `true`. It's captured for completeness.

#### Change 3f: Add `workbaseId` to the `pipeline/adapter.deployed` Inngest event

Find the `await inngest.send` call that fires `pipeline/adapter.deployed` (approximately lines 483–492):
```typescript
      await inngest.send({
...
Find the DB update in Step 7 that currently reads (approximately lines 633–639):
```typescript
...
Add this entire Inngest step **after the closing `}` of the `if (job.workbase_id)` block that contains Step 7b (`update-workbase-active-adapter`), and BEFORE the final `return` statement**:
...
Find the current destructuring (lines 23–29):
```typescript
    const {
      jobId,
      adapterName,
...
The current Step 1 (lines 71–113) uses a `void (await step.run(...))` wrapper pattern. The entire expression including the outer `void (await ...)` must be replaced.
...
Replace the entire `void (await step.run('scale-down-workers', async () => { ... }));` expression with:
...
Replace the entire `step.run('wait-for-workers-terminated', ...)` block (lines 118–132) with:
...
Replace the entire `step.run('scale-up-workers', ...)` block (lines 137–183) with:
...
Replace the entire `step.run('wait-for-workers-ready', ...)` block (lines 188–204) with:
...
> Note: This step is a plain `await step.run(...)` with no variable assignment. The return value is not needed by subsequent steps. Do NOT declare a named variable for this step's result.

#### Change 4f: Replace Step 5 (`verify-adapter-available`) — write to restart log

Replace the entire `step.run('verify-adapter-available', ...)` block (lines 209–238) with:
...
Replace the entire `step.run('mark-endpoints-ready', ...)` block (lines 243–263) with:
...
Also verify that `createServerSupabaseAdminClient` is imported from `@/lib/supabase-server` at the top of `refresh-inference-workers.ts`. It is already present on line 2 — no change needed.
...
The current file (confirmed by reading) exports: `processRAGDocument`, `dispatchTrainingJob`, `dispatchTrainingJobFailureHandler`, `autoDeployAdapter`, `refreshInferenceWorkers`, `autoEnrichConversation`, `buildTrainingSet`, `rebuildTrainingSet`.

Replace the entire file content with the following (adds `restartInferenceWorkers` import and export):
...
```bash
cd "C:\Users\james\Master\BrightHub\BRun\v4-show\src" && npx tsc -p tsconfig.json --noEmit 2>&1 | grep -E "(error|inngest)" | head -30
```
...
Common fixes needed:
- If `loraModulesResult` return type doesn't include `runpodEndpointId` or `saveSuccess`, TypeScript will complain — verify BOTH the `noChange: true` early-return path (Change 3d) AND the `noChange: false` path (Change 3e) include these fields
- The `any` type in `oldJobs.map((j: any) => ...)` is intentional — suppress with `// eslint-disable-next-line @typescript-eslint/no-explicit-any` if linting is strict

> Note: The only pre-existing error in the project is in `src/lib/text-extractor.ts` (pdf-parse types). This is not touched by E02 — do not fix it.

---

### Completion Checklist

- [ ] `auto-deploy-adapter.ts`: `deleteRepo` imported from `@huggingface/hub`
...
- [ ] `auto-deploy-adapter.ts`: Step 4 `noChange: true` and `noChange: false` return paths both include `runpodEndpointId` and `saveSuccess`
...
- [ ] `auto-deploy-adapter.ts`: Step 7a `supersede-old-adapters` added (between Step 7 and Step 7b)
- [ ] `auto-deploy-adapter.ts`: Step 7c `delete-old-hf-repos` added (after Step 7b)
- [ ] `refresh-inference-workers.ts`: `workbaseId` destructured from event
- [ ] `refresh-inference-workers.ts`: Step 1 `void (await ...)` wrapper removed; replaced with `const scaleDownResult = await step.run(...)`
- [ ] `refresh-inference-workers.ts`: Step 1 creates `endpoint_restart_log` row and returns `restartLogId`
- [ ] `refresh-inference-workers.ts`: Steps 2–5 update the log at each stage using `scaleDownResult.restartLogId`
- [ ] `refresh-inference-workers.ts`: Step 4 is a plain `await step.run(...)` with no variable assignment
