...
**What was built:**
- A new **Adapter Detail Page** at `/workbase/[id]/fine-tuning/launch/adapter/[jobId]`
...
3. **E01–E05 database migrations applied** — The `deployment_log` and `adapter_status` columns must exist on `pipeline_training_jobs`, and the `endpoint_restart_log` table must exist.

4. **Your Work Base ID** — Find it in the URL when you navigate to any page in that Work Base: `/workbase/{YOUR_WORKBASE_ID}/...`
...
### Path to feature
```
/workbase/{id}/fine-tuning/launch
```
...
1. Navigate to:
   ```
   http://localhost:3000/workbase/{YOUR_WORKBASE_ID}/fine-tuning/launch
   ```
...
Navigate here by clicking a Completed row in Adapter History (Test 1.2), or navigate directly by replacing `{id}` and `{jobId}` in the URL.
...
**Expected:**
- Navigates back to `/workbase/{id}/fine-tuning/launch`.
...
1. Navigate to the detail page for an adapter deployed **after** the E01 migrations.
...
- The adapter ID string (e.g., `adapter-e8fa481f`) is now in the clipboard. Paste it (Ctrl+V) to verify.
...
- The current active step shows a blue spinning `Loader2` icon.
...
1. While a restart is in progress (or immediately after triggering one), navigate to the Adapter Detail Page for a **different** adapter in the same Work Base and attempt to trigger another restart.

**Expected:**
- The `POST /api/pipeline/adapters/{jobId}/restart` call returns HTTP 409.
...
1. Find a training job that has `status = 'completed'` but no `hf_adapter_path` (i.e., deployment failed before HuggingFace upload).
2. Navigate to its Adapter Detail Page and click **"Restart Endpoint Workers"**.
...
1. Navigate to the Adapter Detail Page for an adapter that was freshly deployed (its auto worker-refresh has completed).
...
- The API call is made to `POST /api/pipeline/adapters/{jobId}/remove`.
...
This test verifies the full round trip from training job completion through the adapter lifecycle.

### Setup
You need a Work Base where you can trigger a new training job. If you cannot train a new job in a test session, use an existing completed job.
...
After the job completes, the `autoDeployAdapter` → `refreshInferenceWorkers` pipeline runs automatically in the background (via Inngest). This typically takes 3–10 minutes total.

**Step 3 — Verify Adapter History entry**
1. Once the job is in "Completed" status, check the Adapter History list.
2. Confirm the entry shows `· View details →` and is clickable.

**Step 4 — Click through to Detail Page**
1. Click the completed entry.
2. Confirm you land on the Adapter Detail Page for the correct job.
...
1. Click **Restart Endpoint Workers**, read the warning, confirm.
2. Watch the step-by-step progress (takes 1–4 minutes).
3. Confirm the verdict card shows **SUCCESS** after completion.
...
2. Verify the success toast and the status badge changes to "Superseded".
3. Attempt to click Refresh Status after removal — the LORA_MODULES row should now show red ✗.
4. Return to `/workbase/{id}/fine-tuning/launch` and confirm the removed adapter's row in Adapter History now shows the "Superseded" badge (if applicable).
...
1. Manually type a fake UUID into the URL:
   ```
   /workbase/{id}/fine-tuning/launch/adapter/00000000-0000-0000-0000-000000000000
   ```
...
1. If the endpoint is currently at 0 workers (e.g., during off-hours auto-scaling), run the Adapter Status Ping.
...
If a previous restart failed mid-way (e.g., workers failed to terminate), navigate to an adapter where `endpoint_restart_log.status = 'failed'`.
...
## Appendix: Feature → URL Quick Reference

| Feature | URL |
|---------|-----|
| Launch Tuning (Adapter History list) | `/workbase/{id}/fine-tuning/launch` |
| Adapter Detail Page | `/workbase/{id}/fine-tuning/launch/adapter/{jobId}` |
| API: Deployment Log | `GET /api/pipeline/jobs/{jobId}/deployment-log` |
| API: Live Status Ping | `GET /api/pipeline/adapters/{jobId}/ping` |
| API: Trigger Restart | `POST /api/pipeline/adapters/{jobId}/restart` |
| API: Poll Restart Progress | `GET /api/pipeline/adapters/{jobId}/restart-status` |
| API: Remove Adapter | `POST /api/pipeline/adapters/{jobId}/remove` |

---

## Appendix: Acceptance Criteria Summary

All of the following must pass before Spec 26 is considered complete.
