...
1. **`'use client'` removed from `useAdapterDetail.ts`** — Both reference hook files (`usePipelineJobs.ts` and `useAdapterTesting.ts`) have no `'use client'` directive. Hook files in this codebase do not use the directive — it is only placed on React component files. The hook file in v1 incorrectly included it.

2. **Duplicate polling removed from `EndpointRestartTool`** — v1's `handleConfirmRestart` created a manual `setInterval` inside the async handler, which ran alongside the `useEffect` interval that already fires whenever `isActive` is true. The manual interval was redundant and has been removed. The `useEffect` approach is sufficient.

3. **Removed misleading TypeScript warning from Common Issues** — v1 warned that `workersOnline` might cause a `number` type error on the `checked` prop. The expression `pingData.workerStatus.ready + pingData.workerStatus.idle > 0` already returns `boolean` (the `> 0` comparison is the boolean conversion). Not a real issue.

4. **`alert-dialog` confirmed available** — `src/components/ui/alert-dialog.tsx` exists. Removed the "check first" caveat from Common Issues.

5. **All `EndpointRestartLog` field names confirmed** from reading `src/types/adapter-detail.ts` (produced by E01): All camelCase fields used in `EndpointRestartTool` (`scaleDownAt`, `workerTerminatedAt`, `scaleUpAt`, `workersReadyAt`, `verificationAt`, `scaleDownSuccess`, `scaleUpSuccess`, `adapterVerified`, `adapterInLoraModules`, `loraModulesSnapshot`, `totalDurationMs`, `triggerSource`, `initiatedAt`, `errorMessage`, `errorStep`) match the type exactly.

6. **`DeploymentLog` snake_case sub-fields confirmed** from reading `src/types/adapter-detail.ts`: `worker_refresh` sub-object uses snake_case (`verification_result`, `verification_error`). The component accesses these correctly.
...
**React Query v5 patterns (confirmed from `src/hooks/usePipelineJobs.ts`):**
```typescript
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
```
Hook files do NOT use `'use client'` — that directive belongs only on component files.
...
3. **Hook files do NOT use `'use client'`** — component files do.
...
> No `'use client'` directive — hook files in this codebase do not use it (confirmed from `usePipelineJobs.ts` and `useAdapterTesting.ts`).
...
> **Field names:** `DeploymentLog.worker_refresh` sub-object uses snake_case (`verification_result`, `verification_error`) — confirmed from `src/types/adapter-detail.ts`.
...
> **Polling:** Uses a single `useEffect` interval that fires every 5s when `isActive` is true. The `handleConfirmRestart` handler does NOT create its own interval — the `useEffect` handles polling as soon as `isActive` becomes true after the mutation succeeds.
...
export function EndpointRestartTool({ jobId, adapterId }: EndpointRestartToolProps) {
  ...
  // Auto-poll every 5s when a restart is active — single polling mechanism
  useEffect(() => {
    if (!isActive) return;
  ...
  const handleConfirmRestart = async () => {
    ...
    try {
      ...
      // useEffect above picks up polling as soon as isActive becomes true
    } catch (err: unknown) {
...
Fix any errors in the new files. Known issues that are NOT present (confirmed from codebase reads):
- `workersOnline` type is already `boolean | null` — no cast needed
- `alert-dialog` is available at `@/components/ui/alert-dialog`
- All `EndpointRestartLog` field names are camelCase — confirmed correct
...
- [ ] `src/hooks/useAdapterDetail.ts` created — NO `'use client'` directive — exports `adapterDetailKeys`, `useDeploymentLog`, `useAdapterPing`, `useRestartStatus`, `useTriggerRestart`, `useRemoveAdapter`
...
- [ ] `src/components/pipeline/EndpointRestartTool.tsx` created — single `useEffect` polling only, confirmation dialog with cross-workbase warning
