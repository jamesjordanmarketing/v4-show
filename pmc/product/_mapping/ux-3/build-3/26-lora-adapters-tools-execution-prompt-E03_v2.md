...
1. **New Step 0: Update `client.ts`** — `InngestEvents` in `src/inngest/client.ts` is missing `workbaseId` in the `pipeline/adapter.deployed` event (added to the actual event payload in E02) and is missing the new `pipeline/endpoint.restart.requested` event entirely. Added as Step 0 to keep the type documentation accurate. Note: since `inngest` is instantiated without the `InngestEvents` generic, TypeScript will not error at compile time — this is a documentation correctness fix.

2. **Route 4 `logErr` variable conflict fixed in code** — v1 mentioned this in "Common Issues" but did not fix it in the actual code block. In v2, the restart log query error variable is renamed to `logFetchErr` directly in the code.

3. **Removed false `updated_at` warning from Common Issues** — v1 warned that `updated_at` on `pipeline_inference_endpoints` may need an `as any` cast. Confirmed false: E02's `supersede-old-adapters` and `mark-endpoints-ready` steps both update `pipeline_inference_endpoints.updated_at` and the TypeScript compilation passes with zero errors. Not an issue.

4. **`user_id` column confirmed** — E01's `endpoint_restart_log` DDL includes `user_id UUID REFERENCES auth.users(id)`. The Route 3 insert with `user_id: user.id` is correct and matches the schema.

5. **All import paths and auth patterns confirmed** from reading the actual files:
   - `requireAuth` and `createServerSupabaseAdminClient` are both confirmed exported from `@/lib/supabase-server`
   - `@/inngest/client` path confirmed (`src/inngest/client.ts` exists)
   - `mapDbRowToRestartLog` confirmed exported from `@/types/adapter-detail`

---

## What This Prompt Builds

One update to an existing file and five new Next.js 14 App Router API routes:

| Step | File | Change |
|------|------|--------|
| 0 | `src/inngest/client.ts` | Update `InngestEvents` type (add missing event types) |
| 1 | `/api/pipeline/jobs/[jobId]/deployment-log` | GET — Read `deployment_log` JSONB from a training job |
| 2 | `/api/pipeline/adapters/[jobId]/ping` | GET — Live RunPod status ping (LORA_MODULES check + inference test) |
| 3 | `/api/pipeline/adapters/[jobId]/restart` | POST — Trigger manual worker restart via Inngest |
| 4 | `/api/pipeline/adapters/[jobId]/restart-status` | GET — Poll latest restart log for a job |
| 5 | `/api/pipeline/adapters/[jobId]/remove` | POST — Remove adapter from RunPod LORA_MODULES |
...
**Auth pattern (confirmed from `src/app/api/pipeline/jobs/[jobId]/route.ts`):**
```typescript
...
- `src/app/api/pipeline/jobs/[jobId]/` — has `route.ts` only; no sub-routes yet
- `src/app/api/pipeline/adapters/` — has `status/`, `test/`, `deploy/`, `rate/` (all static segments, no `[jobId]` yet)
...
**Files to modify:**
1. `src/inngest/client.ts` — update `InngestEvents` type
...
### Step 0: Update `src/inngest/client.ts` — Add Missing Event Types

Read `src/inngest/client.ts` first. Then make two surgical changes:

#### Change A: Add `workbaseId` to `pipeline/adapter.deployed`

Find the current `pipeline/adapter.deployed` declaration:
```typescript
...
#### Change B: Add `pipeline/endpoint.restart.requested` event

Add this entry to `InngestEvents` after `pipeline/adapter.deployed`:
...
### Step 1: Read Existing Route for Pattern Confirmation

Read `src/app/api/pipeline/jobs/[jobId]/route.ts` to confirm the auth pattern before writing any new routes.
...
4. Creates an `endpoint_restart_log` row with `trigger_source='manual'`, `status='initiated'`, and `user_id`
...
> **Variable naming:** The job query error is `jobErr` and the log query error is `logFetchErr` — different names to avoid a TypeScript redeclaration error.
...
```bash
cd "C:\Users\james\Master\BrightHub\BRun\v4-show\src" && npx tsc -p tsconfig.json --noEmit 2>&1 | head -30
```

Fix any errors in the new route files. Pre-existing errors elsewhere are acceptable.

Known issues to watch for:
- `user!.id` non-null assertion — required because `requireAuth` returns `User | null` even after the guard check. The `!` operator is used throughout this spec's routes.
- `inngest.send` with `pipeline/endpoint.restart.requested` — compiles without error because `inngest` is not typed with `InngestEvents` generic (the type is documentation only).

---

### Completion Checklist

- [ ] `src/inngest/client.ts`: `workbaseId` added to `pipeline/adapter.deployed` event type
- [ ] `src/inngest/client.ts`: `pipeline/endpoint.restart.requested` event type added
...
- [ ] `restart-status/route.ts` uses `logFetchErr` (not `logErr`) for the log query error variable
