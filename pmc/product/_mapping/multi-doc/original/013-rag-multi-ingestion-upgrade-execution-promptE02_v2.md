...
**Change Log (v2):** Corrected line numbers to match post-E01 codebase state, expanded Task 1 to fix ALL 7 `maxTokens` references across 3 methods (not just `readDocument`), corrected `parseJsonResponse` location to lines 28-95.

---

## Overview

This prompt implements the 6 new extraction methods in the Claude LLM Provider — one for each pass of the multi-pass ingestion pipeline. Each method includes a carefully designed prompt for its specific extraction task, uses the per-pass model routing from the updated `RAG_CONFIG`, and returns a strongly-typed result. It also fixes ALL existing methods that reference `RAG_CONFIG.llm.maxTokens` to use the new object shape.
...
2. Fix ALL 7 references to `RAG_CONFIG.llm.maxTokens` to use `RAG_CONFIG.llm.maxTokens.default`
...
2. **The `parseJsonResponse<T>()` helper already exists** (lines 28-95). It is a **standalone function** (not a class method). Reuse it for all new methods.
...
### Task 1: Fix ALL `maxTokens` References + Update Imports

Since E01 changed `RAG_CONFIG.llm.maxTokens` from a number to an object, **ALL 7 references** in the file now need to be updated. There are references in 3 methods:

#### 1a. Update imports (lines 1-14)
...
#### 1b. Fix `readDocument()` — 5 references (lines 236, 247, 259, 274, 284)

| Line | Current | Replace With |
|------|---------|-------------|
| 236 | `maxTokens: RAG_CONFIG.llm.maxTokens,` | `maxTokens: RAG_CONFIG.llm.maxTokens.default,` |
| 247 | `max_tokens: RAG_CONFIG.llm.maxTokens,` | `max_tokens: RAG_CONFIG.llm.maxTokens.default,` |
| 259 | `` `[readDocument] Claude hit max_tokens limit (${RAG_CONFIG.llm.maxTokens}). ` `` | `` `[readDocument] Claude hit max_tokens limit (${RAG_CONFIG.llm.maxTokens.default}). ` `` |
| 274 | `maxTokens: RAG_CONFIG.llm.maxTokens,` | `maxTokens: RAG_CONFIG.llm.maxTokens.default,` |
| 284 | `` `  - Max tokens configured: ${RAG_CONFIG.llm.maxTokens}` `` | `` `  - Max tokens configured: ${RAG_CONFIG.llm.maxTokens.default}` `` |

#### 1c. Fix `refineKnowledge()` — 1 reference (line 333)
...
#### 1d. Fix `generateResponse()` — 1 reference (line 413)
...
**Total: 7 occurrences of `RAG_CONFIG.llm.maxTokens` → `RAG_CONFIG.llm.maxTokens.default`**

---

### Task 2: Add Pass 1 — `analyzeDocumentStructure()`

Add this method to the `ClaudeLLMProvider` class (before the closing `}` of the class, currently at line 472):
...
1. **TypeScript compilation:** Run `npx tsc --noEmit -p src/tsconfig.json` from the `v4-show/` directory. There should be NO type errors in `claude-llm-provider.ts` now (all 7 `maxTokens` references fixed).
...
4. **No remaining `RAG_CONFIG.llm.maxTokens` without `.default` or `.passName`**: 
```bash
cd "c:/Users/james/Master/BrightHub/BRun/v4-show" && grep -n "RAG_CONFIG.llm.maxTokens[^.]" src/lib/rag/providers/claude-llm-provider.ts
```
   Expected: No matches (all references should now end with `.default` or a pass-specific key).

5. **File size:** `claude-llm-provider.ts` should now be approximately 870-900 lines (was 473).
...
|------|--------|-------------|
| `src/lib/rag/providers/claude-llm-provider.ts` | MODIFY | Add 6 new methods, update imports, fix ALL 7 maxTokens references |
