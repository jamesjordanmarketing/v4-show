...
**Version:** 3.0 (cross-referenced against post-E02 codebase, Feb 16 2026)  
...
> [!NOTE]
> **What changed from v1?** The original E03 was validated against the post-E02 codebase.
> All file line counts, import paths, type shapes, and method signatures match the live code.  
> The following corrections were applied:

| # | Issue | Fix |
|---|-------|-----|
| 1 | Verification checklist item 4 listed **13 step names** but called them "12 steps" | Fixed: pipeline has **13 `step.run()` calls** (12 numbered + finalize). Checklist now accurate. |
| 2 | Overview said "8 new exported functions" but only 7 are `export`ed; `generateContentHash` + `extractQualifiesRule` are private | Fixed: clarified "7 new exported functions + 2 private helpers + 1 existing function modified" |
| 3 | `uploadDocumentFile()` update block described as "around line 237" | Corrected: update block spans **lines 230-239** (comment at 230, `.update()` at 233-238, `.eq()` at 239) |
| 4 | `document_metadata` field referenced in Inngest pipeline `step.run('finalize')` but not in the `rag_documents` table schema | Removed: finalize step only writes columns that exist in the schema |
| 5 | The import from `@/lib/rag/services/rag-db-mappers` in the Inngest pipeline must match actual exports | Verified: `mapRowToDocument`, `mapRowToSection`, `mapRowToFact` are all exported from `rag-db-mappers.ts` |
| 6 | `StructureAnalysisResult.expertQuestions` field type is `ExpertQuestionGeneration[]` (not inline) | Verified: matches `ExpertQuestionGeneration` interface (lines 397-401 in `rag.ts`) |
| 7 | Embedding service import style | `RAGFact` import must merge with existing `@/types/rag` import on line 4 (which already imports `RAGEmbeddingSourceType`, `RAGEmbeddingTier`) |
| 8 | Inngest pipeline Step 10 calls `provider.generateContextualPreamble()` | Verified: method exists at line 301 of `claude-llm-provider.ts` (one of the 8 original methods) |
| 9 | `RAG_CONFIG` import is needed in ingestion service for `storage.bucket` | Already imported at line 14. No change needed. |
| 10 | Inngest pipeline uses `doc.fastMode` which requires `RAGDocument.fastMode` field | Verified: `fast_mode` exists in `RAGDocumentRow` (line 546) and `mapRowToDocument` maps it. |
| 11 | `qualifiesRule` on `FactExtraction` is optional (`qualifiesRule?: string`) | `policyResultToFacts` accesses `exc.qualifiesRule` on `PolicyExtractionResult.exceptions` — this is the *result interface* field (required), not the `FactExtraction` field. Correct as-is. |

---

## Overview

This prompt implements the core ingestion refactor: new helper functions in the ingestion service, document fingerprinting, the `buildEnrichedEmbeddingText()` function, and the complete 13-step Inngest multi-pass pipeline that orchestrates all 6 passes.

**What This Section Creates / Modifies:**
1. `src/lib/rag/services/rag-ingestion-service.ts` — 7 new exported functions + 2 private helpers + 1 existing function modified:
   ...
   - `generateContentHash()` — private, SHA-256 fingerprint
   - `extractQualifiesRule()` — private, regex parser for exception→rule linking
...
3. `src/inngest/functions/process-rag-document.ts` — complete rewrite: 13-step multi-pass pipeline
...
- `generateContextualPreamble()` exists at line 301 (original pre-E01 method, still used in new pipeline Step 10)
...
Update the existing import from `@/types/rag` (currently lines 6-13) to include multi-pass types:
```typescript
import type {
  RAGDocument,
  RAGSection,
...
Find the section in `uploadDocumentFile()` that updates the document record after text extraction (lines 230-239). It currently looks like:
...
Add this function **after** the existing `deleteDocumentEmbeddings` function at the bottom of the file (after line 195).

**IMPORTANT:** The `RAGFact` import must be merged with the existing import from `@/types/rag` at line 4. Update line 4 from:
```typescript
...
**Replace the entire file** with the following 13-step multi-pass pipeline:
...
1. **TypeScript compilation:** Run `npx tsc --noEmit -p src/tsconfig.json` from project root. Should have no errors.
...
4. **Inngest pipeline has 13 `step.run()` calls:** `fetch-document`, `pass-1-structure`, `store-sections`, `pass-2-policies`, `pass-3-tables`, `pass-4-glossary`, `pass-5-narrative`, `pass-6-verification`, `link-relationships`, `store-expert-questions`, `generate-preambles`, `generate-embeddings`, `finalize`
...
|------|--------|-------------|
| `src/lib/rag/services/rag-ingestion-service.ts` | MODIFY | Add 7 exported functions + 2 private helpers + crypto import + content hash in upload |
| `src/lib/rag/services/rag-embedding-service.ts` | MODIFY | Add `RAGFact` to import + `buildEnrichedEmbeddingText()` function |
| `src/inngest/functions/process-rag-document.ts` | REPLACE | 13-step multi-pass pipeline |
