...
**Prerequisites:** E01 complete (all 8 RAG tables created, storage bucket exists)
**Status:** Ready for Execution
**Changes from v1:** Updated prerequisites, added environment variable checks, clarified E01 completion status
...
# OpenAI API (ADD THIS if not present)
OPENAI_API_KEY=sk-...
...
# Database URL (already set - for SAOL operations)
DATABASE_URL=postgresql://postgres:[PASSWORD]@db.hqhtbxlgzysfbekexwku.supabase.co:5432/postgres
```

**⚠️ ACTION REQUIRED:** If `OPENAI_API_KEY` is not in `.env.local`, obtain one from https://platform.openai.com/api-keys and add it before proceeding.
...
- ✅ All 8 RAG tables created: `rag_knowledge_bases`, `rag_documents`, `rag_sections`, `rag_facts`, `rag_expert_questions`, `rag_embeddings`, `rag_queries`, `rag_quality_scores`
- ✅ RLS enabled on all 8 tables with 32 policies (4 per table)
- ✅ 16 standard indexes + 1 HNSW vector index created
- ✅ `updated_at` triggers applied to 5 tables
- ✅ Storage bucket `rag-documents` created (private, 50MB limit)
- ✅ DATABASE_URL configured and SAOL operational

**Note:** E01 was executed via manual SQL script in Supabase Dashboard due to initial DATABASE_URL configuration issue (now resolved). All database objects are successfully created and verified.
...
- **No RAG code exists yet** — all files in this prompt are new
...
```bash
cd "c:/Users/james/Master/BrightHub/brun/v4-show" && node -e "require('dotenv').config({path:'.env.local'});console.log('ANTHROPIC_API_KEY:',process.env.ANTHROPIC_API_KEY?'✓ SET':'✗ MISSING');console.log('OPENAI_API_KEY:',process.env.OPENAI_API_KEY?'✓ SET':'✗ MISSING');console.log('DATABASE_URL:',process.env.DATABASE_URL?'✓ SET':'✗ MISSING');"
```

**Expected:** All three show "✓ SET"

**If OPENAI_API_KEY is missing:** Obtain from https://platform.openai.com/api-keys and add to `.env.local`
...
**If `@anthropic-ai/sdk` not found:** Run `npm install @anthropic-ai/sdk`
...
- [ ] Environment variables verified (ANTHROPIC_API_KEY, OPENAI_API_KEY, DATABASE_URL)
...
- If OPENAI_API_KEY is missing, create one at https://platform.openai.com/api-keys
- Environment variables must be in `.env.local` (not `.env` or `.env.previous.local`)
...
7. **Verify OPENAI_API_KEY exists** before proceeding with verification steps.

---

**End of E02 Prompt v2**
