...
**Changes from v1:**
- Fixed Check 2 line range: 67–96 → 66–104 (includes comment line + catch block)
- Fixed Check 3 RPC call: add `filter_run_id: null` to align with the updated 7-param signature
- Fixed `runSingleTest` queryRAG call line reference: 209 → 208
- Fixed `runSingleTest` call line in route.ts: 70–71 → 76
- Fixed unused `user` variable in runs GET endpoint (destructure without it)
- Updated `golden-set-definitions.ts` line count: 229 → 230 (E010 added `embeddingRunId?`)
...
- `rag_test_reports` table exists (18 columns — note: spec said 17, actual is 18 including `id`)
- `rag_embeddings.run_id` column exists (UUID, nullable)
- `match_rag_embeddings_kb` accepts `filter_run_id uuid DEFAULT NULL` as its 7th parameter
...
- `C:\Users\james\Master\BrightHub\BRun\v4-show\src\lib\rag\testing\golden-set-definitions.ts` (230 lines) — contains types and `CANONICAL_DOCUMENT_ID`
...
The `requireAuth` function returns `{ user, response }`. If `response` is non-null, return it immediately (401 unauthorized). Only destructure `user` if you actually use it in the handler body.
...
#### 1a. Enhance "Check 2: Embeddings exist" block (lines 66–104)

The current Check 2 block (lines 66–104, starting with `// Check 2:` comment and ending with the closing `}` of the catch) queries embeddings filtered by `document_id` and does a simple count by tier. Enhance it to also show creation time range, run counts, and untagged counts.

**Current code (lines 66–104):**
...
try {
    ...
    if (error) {
        ...
        // Count by tier
        const tier1 = embeddings?.filter((e: { tier: number }) => e.tier === 1).length || 0;
        const tier2 = embeddings?.filter((e: { tier: number }) => e.tier === 2).length || 0;
        const tier3 = embeddings?.filter((e: { tier: number }) => e.tier === 3).length || 0;
...
**Key points:**
- Changed `.select('id, tier')` to `.select('id, tier, created_at, run_id')` and added `.order('created_at', { ascending: true })`
- Changed typed filter lambdas `(e: { tier: number })` to `(e: any)` to accommodate the new fields
- Added time range and run tag analysis before the final `checks.push`

#### 1b. Update Check 3 RPC call to include `filter_run_id`

After E010, `match_rag_embeddings_kb` now has a 7th parameter `filter_run_id uuid DEFAULT NULL`. The Check 3 preflight call (around line 112) does not yet pass this parameter. Update it for alignment.

**Current Check 3 RPC call (lines 112–119):**
...
#### 1c. Add `runId` parameter to `runSingleTest`
...
**And update the `queryRAG` call inside `runSingleTest` (around line 208):**
...
#### 1d. Add `getEmbeddingRuns` function

At the bottom of `test-diagnostics.ts` (after line 277, the current end of file), add a new exported function and its interface:
...
**Note:** `user` is intentionally not destructured here — this endpoint only needs auth presence, not the user identity.
...
**Note:** The GET handler only needs auth presence; `user` is not destructured to avoid unused-variable warnings.
...
**Note:** `user` is intentionally not destructured — this endpoint only needs auth presence, not the user identity.
...
2. **Files modified (2):**
   - `src/lib/rag/testing/test-diagnostics.ts` — enhanced Check 2 (run tags + time range), Check 3 RPC updated, `runSingleTest` has `runId`, new `getEmbeddingRuns` function
...
|------|--------|-------------|
| `src/lib/rag/testing/test-diagnostics.ts` | Modify | Enhanced preflight Check 2 (time range + run tags), Check 3 RPC aligned, `runId` on `runSingleTest`, new `getEmbeddingRuns` |
...
- [ ] Preflight Check 3 RPC call includes `filter_run_id: null`
