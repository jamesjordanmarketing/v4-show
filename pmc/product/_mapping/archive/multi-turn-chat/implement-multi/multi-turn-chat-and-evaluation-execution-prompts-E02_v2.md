...
**Changes from v1:** Renamed service file to avoid conflict with legacy system
...
1. Service file: `src/lib/services/multi-turn-conversation-service.ts` (~800 lines)
2. Update: `src/lib/services/test-service.ts` (add exports for helper functions)
3. Update: `src/lib/services/index.ts` (add multi-turn service export)
...
**Key Changes from v1:**
- ✅ Service file renamed to `multi-turn-conversation-service.ts` (avoids conflict with existing `conversation-service.ts` used by legacy system)
- ✅ Added step to export helper functions from `test-service.ts`
...
- `src/lib/services/conversation-service.ts` — LEGACY conversation service (now uses `legacy_conversation_turns`)

### File Naming

**IMPORTANT:** The new multi-turn service is named `multi-turn-conversation-service.ts` to avoid conflict with the existing `conversation-service.ts` which handles the legacy conversation generation system.

### Imports

The service uses these existing functions from other services:
- `callInferenceEndpoint`, `getEndpointStatus` from `./inference-service` ✅ (verified exist)
- `evaluateWithClaude`, `compareEvaluations`, `getEvaluationPrompt`, `getEmotionalArcsContext` from `./test-service` ⚠️ (exist but need to be exported)
...
- `conversation_turns` table exists (new multi-turn schema) ✅
- `legacy_conversation_turns` table exists (renamed from old `conversation_turns`) ✅
- `src/types/conversation.ts` exists with all types ✅

### Task 1: Export Helper Functions from Test Service

**Update file:** `src/lib/services/test-service.ts`

Add these exports at the end of the file (after the existing exports):
...
### Task 2: Create Multi-Turn Conversation Service

**Create file:** `src/lib/services/multi-turn-conversation-service.ts`
...
### Task 3: Export from Services Index

**Update file:** `src/lib/services/index.ts`

Add to the end of the exports:
...
- [ ] `src/lib/services/test-service.ts` exports helper functions
- [ ] `src/lib/services/multi-turn-conversation-service.ts` created with all functions
- [ ] `src/lib/services/index.ts` exports multi-turn conversation service functions
...
| File | Action | Purpose |
|------|--------|---------|
| `src/lib/services/multi-turn-conversation-service.ts` | Created | Multi-turn service layer with CRUD + evaluation |
| `src/lib/services/test-service.ts` | Modified | Export evaluation helper functions |
| `src/lib/services/index.ts` | Modified | Export new multi-turn service |
...
## Key Differences from E02_v1

1. **Service File Renamed:** `conversation-service.ts` → `multi-turn-conversation-service.ts` to avoid conflict with legacy system
2. **Added Task 1:** Export helper functions from `test-service.ts`
3. **Updated Documentation:** Clarified coexistence with legacy system
4. **Verified Imports:** Confirmed all dependencies exist in current codebase

---

**END OF E02_v2 PROMPT**
