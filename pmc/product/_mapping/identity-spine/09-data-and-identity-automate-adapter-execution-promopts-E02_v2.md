...
- Installed `tar-stream ^3.1.7` and `@types/tar-stream ^3.1.4` npm packages ✓
...
**Confirmed `.env.local` state entering E02:**
- `GPU_CLUSTER_API_KEY` — RunPod API key (used by `auto-deploy-adapter.ts` as `RUNPOD_API_KEY`)
- `GPU_CLUSTER_API_URL` — Training endpoint URL (`https://api.runpod.io/v2/ei82ickpenoqlp`)
- `INFERENCE_API_URL` — Inference endpoint URL (`https://api.runpod.io/v2/780tauhj7c126b`); used as the vLLM hot-reload target in Step 5 (non-fatal)
- `RUNPOD_INFERENCE_ENDPOINT_ID=ei82ickpenoqlp` — The endpoint ID whose `LORA_MODULES` env var gets updated in Step 4
...
**What to confirm:**
- The file exports `inngest` (Inngest client instance) and `InngestEvents` type
- `InngestEvents` includes `rag/document.uploaded` and `pipeline/job.created`
- `pipeline/adapter.ready` is **NOT yet present** (you will add it)
- The file ends with `'pipeline/job.created': { data: { jobId: string; userId: string; }; }; };` — this is the exact text you will replace in Step 2
...
**What to confirm:**
- It imports `processRAGDocument` and `dispatchTrainingJob`/`dispatchTrainingJobFailureHandler`
- `inngestFunctions` array contains those three functions
- `autoDeployAdapter` is **NOT yet imported or registered**

**Expected current content (the file has JSDoc comments — this is normal):**
```typescript
/**
...
**What to confirm:** `auto-deploy-adapter.ts` does **NOT yet exist** (you will create it)
...
**What to confirm:**
- `createServerSupabaseAdminClient()` is exported (line ~14)
- It uses `SUPABASE_SERVICE_ROLE_KEY` and `createClient` from `@supabase/supabase-js` directly (not the SSR client)
- The import path `@/lib/supabase-server` resolves to this file — this is the correct import to use in `auto-deploy-adapter.ts`
 ...
 *   RUNPOD_INFERENCE_ENDPOINT_ID — "ei82ickpenoqlp" (endpoint to update LORA_MODULES on)
...
export const autoDeployAdapter = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // =====================================================
    // Step 4: Update RunPod inference endpoint LORA_MODULES via GraphQL API
    // Target endpoint: RUNPOD_INFERENCE_ENDPOINT_ID = ei82ickpenoqlp
    // =====================================================
...
### Current actual content (verified against codebase — includes JSDoc comments)
...
/**
 * Function Export for Direct Import
 *
 * This allows importing specific functions elsewhere if needed:
 * import { processRAGDocument } from '@/inngest/functions';
 */
export { processRAGDocument, dispatchTrainingJob, dispatchTrainingJobFailureHandler };
```
...
| Error | Fix |
|-------|-----|
| `Cannot find module 'tar-stream'` | Run `npm install tar-stream @types/tar-stream` (E01 step — should already be done) |
...
| `tarStream.extract is not a function` | tar-stream v3 ESM — ensure dynamic import is used exactly as written (`await import('tar-stream')`) |
...
**No database changes in E02. No new env vars required — all are already in `.env.local` from E01.**
...
| Area | v1 | v2 |
|------|----|----|
| "What E01 completed" — HF_TOKEN | "Added env var placeholder" | Corrected: was already present with real value; left untouched |
| "What E01 completed" — vars added | Listed all 5 as placeholders | Corrected: 4 added (`HF_ORG`, `HF_ADAPTER_REPO_PREFIX`, `RUNPOD_INFERENCE_ENDPOINT_ID`, `WEBHOOK_SECRET`) |
| Step 1b "current expected content" | Simplified (no JSDoc) | Updated to match actual file including JSDoc comment blocks |
| Step 4 "current actual content" | Simplified (no JSDoc) | Updated to match actual file including JSDoc comment blocks |
| Step 5 vLLM note | No note on INFERENCE_API_URL value | Added note: value is RunPod serverless URL; hot reload non-200 is expected and non-fatal |
| `auto-deploy-adapter.ts` file header | Lists env vars generically | Explicitly documents confirmed `.env.local` var names and values |
