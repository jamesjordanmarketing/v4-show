...
| # | Update | Reason |
|---|--------|--------|
| 1 | Phase C4 updated: `documents/route.ts` and `documents/[id]/route.ts` removed from deletion list | E03 (Changes 5.7 + 5.8) secured these as HIGH priority H10/H11 routes — they are live, not dead Chunks Module code |
| 2 | Phase C4: `src/app/api/documents/` directory NOT deleted | Two secured route files remain in that directory |
| 3 | Phase C6: `documents` table removed from drop list (14 tables, not 15) | The secured `documents/route.ts` and `documents/[id]/route.ts` both access the `documents` table |
| 4 | Phase V2 verification query updated | No longer checks for `documents` in the "chunks tables gone" check |
| 5 | "What E04 expects" item #4 removed | That item said "documents table no longer exists" — no longer true |
| 6 | Division of Work updated | E03 was executed at v2 scope (H1–H15, M1–M13, all RAG gaps) |
| 7 | Preflight 0.5 note added | Notes that documents/route.ts and documents/[id]/route.ts now exist as E03 secured artifacts |
| 8 | Summary updated | Files deleted count and table count corrected |
...
| **E02** (complete, v2) | CRITICAL route security fixes (C1–C18 + 11 C1-adj), batch-job-service + conversation-service refactoring |
| **E03** (complete, v2) | HIGH route/service fixes (H1–H15), MEDIUM fixes (M1–M13), RAG gap fixes (GAP-R1/R2/R3) |
...
# Also note: documents/route.ts and documents/[id]/route.ts are E03 SECURED artifacts — do NOT delete these.
...
# The documents/route.ts and documents/[id]/route.ts ARE expected to exist (secured in E03 as H10/H11).
# Only the sub-routes (upload, process, status) will be deleted in Phase C4.
```
...
(async () => {
  ...
  console.log('\nExpected: training_files=1, datasets=1, pipeline_training_jobs=1, conversations=242, training_file_conversations=242, documents=0');
})();
```
...
- `documents`: 0 rows (cleared; table structure kept — table is used by secured routes from E03)
...
> **Important:** `src/app/api/documents/route.ts` and `src/app/api/documents/[id]/route.ts` are **NOT** part of the Chunks Module removal. These were secured in E03 (Changes 5.7/H10 and 5.8/H11) as live HIGH-priority routes and must be preserved. Only the three sub-routes (`upload`, `process`, `status`) are being deleted in Phase C4.
...
### Phase C4 — Delete 3 Dead Documents Sub-Routes + 3 Workflow API Routes

> **⚠️ v2 change:** `src/app/api/documents/route.ts` and `src/app/api/documents/[id]/route.ts` are **NOT deleted here** — they were secured in E03 as live H10/H11 routes. Only the three dead Chunks Module sub-routes are removed from this directory.

**Documents Chunks Module sub-routes (verify each exists, then delete):**
- `src/app/api/documents/upload/route.ts`
- `src/app/api/documents/process/route.ts`
- `src/app/api/documents/status/route.ts`

> **Do NOT delete** `src/app/api/documents/route.ts` or `src/app/api/documents/[id]/route.ts`.
> **Do NOT delete** `src/app/api/documents/` directory — it still contains the two secured route files.

**Workflow API routes (verify each exists, then delete):**
...
**Acceptance Criteria:**
- `src/app/api/documents/upload/route.ts`, `process/route.ts`, `status/route.ts` do not exist
- `src/app/api/documents/route.ts` and `src/app/api/documents/[id]/route.ts` still exist (E03 artifacts)
...
### Phase C6 — Drop 14 Database Tables

> **⚠️ v2 change:** `documents` table is **NOT dropped** here. It is an active table used by the E03-secured `documents/route.ts` and `documents/[id]/route.ts` routes (Changes 5.7/H10 and 5.8/H11). The 14 tables below are pure Chunks Module infrastructure with no live code dependencies.
...
AND table_name IN ('chunks','chunk_dimensions','chunk_runs','chunk_extraction_jobs','workflow_sessions','workflow_metadata','processing_jobs','categories','tags','document_tags','document_categories','tag_dimensions','custom_tags','dimension_metadata')
ORDER BY table_name
\`,transport:'pg'});
if(r.rows.length===0){console.log('All 14 chunks module tables dropped successfully.');}
else{console.log('Tables still present:',r.rows.map(r=>r.table_name));}
})();"
```

**Expected:** `All 14 chunks module tables dropped successfully.`

**Tables NOT dropped (preserve these):**
- `documents` — active table used by E03-secured documents API routes (H10/H11)
...
**Acceptance Criteria:**
- 0 of the 14 listed tables remain in the database
- `documents`, `prompt_templates`, and `orphaned_conversations` still exist
...
**Expected:** Zero output. The only pre-existing error is in `lib/file-processing/text-extractor.ts` (pdf-parse type — pre-existing, predates E01). No new errors from the Chunks Module removal.
...
# V2 — Chunks-specific tables gone (does NOT include documents — that table is preserved)
cd "c:/Users/james/Master/BrightHub/BRun/v4-show/supa-agent-ops" && node -e "
require('dotenv').config({path:'../.env.local'});const saol=require('.');
(async()=>{const r=await saol.agentExecuteSQL({sql:\`SELECT COUNT(*) AS c FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('chunks','chunk_dimensions','chunk_runs','chunk_extraction_jobs','workflow_sessions','workflow_metadata','processing_jobs','categories','tags','document_tags','document_categories','tag_dimensions','custom_tags','dimension_metadata')\`,transport:'pg'});console.log('Chunk-specific tables remaining:',r.rows[0].c);})();"
# Expected: 0
...
# Expected: no output (text-extractor.ts error is pre-existing, filtered out)
...
# V8 — documents table still exists and documents API routes intact (E03 artifacts)
cd "c:/Users/james/Master/BrightHub/BRun/v4-show/supa-agent-ops" && node -e "
require('dotenv').config({path:'../.env.local'});const saol=require('.');
(async()=>{const r=await saol.agentExecuteSQL({sql:\`SELECT COUNT(*) AS c FROM information_schema.tables WHERE table_schema='public' AND table_name='documents'\`,transport:'pg'});console.log('documents table exists:',r.rows[0].c === '1' ? 'YES' : 'NO');})();"
# Expected: YES

grep -n "requireAuth" src/app/api/documents/route.ts src/app/api/documents/\[id\]/route.ts
# Expected: requireAuth present in both files (E03 H10/H11 artifacts)
```
...
> The `documents` storage bucket may contain files uploaded via the old Chunks Module workflow. Since the upload page and workflow routes are gone, no new files will be added. The bucket can be manually emptied at any time via: Supabase Dashboard → Storage → `documents` → Select All → Delete. This is deferred — it does not block any subsequent prompts.
...
| Chunks Module (14) | `chunk_dimensions`, `chunk_extraction_jobs`, `chunk_runs`, `chunks`, `document_tags`, `document_categories`, `workflow_sessions`, `workflow_metadata`, `processing_jobs`, `categories`, `tag_dimensions`, `tags`, `custom_tags`, `dimension_metadata` |

> **Not dropped (v2 update):** `documents` table is preserved — used by E03-secured H10/H11 routes.
...
- `documents` (data cleared in Phase E; table structure and secured routes remain)
...
| API Routes (6) | `documents/upload`, `documents/process`, `documents/status`; `workflow/*` (3); `conversations/by-chunk/*`, `conversations/[id]/link-chunk`, `conversations/[id]/unlink-chunk` |
...
> **Not deleted (v2 update):** `documents/route.ts` and `documents/[id]/route.ts` — these are E03 H10/H11 secured artifacts.
...
4. `documents` table exists with 0 rows (data cleared in Phase E; table structure preserved for secured E03 routes)
5. Chunks Module code has been removed (no compile errors)
6. TypeScript compiles cleanly (only the pre-existing pdf-parse error in `text-extractor.ts` remains)
