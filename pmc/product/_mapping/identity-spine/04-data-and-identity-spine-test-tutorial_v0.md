...
### 0.1 Confirm the app is running
...
### 0.2 Confirm environment

- `.env.local` exists in `v4-show/` with `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `ANTHROPIC_API_KEY`.
...
5. On successful login you are redirected to `/dashboard`.
...
```javascript
const { data } = await fetch('/api/conversations', { credentials: 'include' }).then(r => r.json());
// Or use any auth-required endpoint and inspect the 401 body if not signed in.
```

Alternatively, run this SAOL query after you have created at least one record:
...
1. On the `/conversations` list, find a conversation with status `generated`.
2. Open the conversation preview (click the row).
3. Click **Enrich**.
4. Observe status change: `generated` → `enrichment_processing` → `enrichment_complete`.
5. After completion, the preview shows enrichment metadata and a validation report.

**Route called:** `POST /api/conversations/{id}/enrich`

**To check enrichment status:**

Navigate to: `GET /api/conversations/{id}/enrich` (or observe via the preview modal).
...
1. On `/conversations`, select a conversation with `enrichment_status = 'enrichment_complete'`.
2. Open the status panel.
3. Click **Approve** or **Reject**.
4. For rejection, enter an optional reason.
5. Observe status changes to `approved` or `rejected`.
...
1. On `/conversations`, click the download icon on any conversation.
2. Select **Raw** or **Enriched** (enriched requires enrichment to be complete).
3. A signed URL is returned. The browser downloads the JSON file.
...
5. You are redirected to `/batch-jobs/{jobId}` (or navigate there manually).
...
1. Navigate to `/batch-jobs` to see all batch jobs.
2. Click a job to go to `/batch-jobs/{id}`.
...
- `GET /api/batch-jobs/{id}/cancel` → `POST /api/batch-jobs/{id}/cancel` — to cancel
...
1. Navigate to `/conversations/failed`.
...
1. Navigate to `/api/generation-logs` (direct API call or via browser DevTools while on `/conversations`).
2. Apply filters via query params: `conversationId`, `status`, `modelUsed`, `dateFrom`, `dateTo`.
3. Get cost summary: `GET /api/generation-logs/stats?type=cost&startDate=2026-01-01&endDate=2026-12-31`
4. Get performance metrics: `GET /api/generation-logs/stats?type=performance`
...
5. Click **Create**.
6. Observe: new training file appears on `/training-files`.
...
1. On `/training-files`, click **Download JSON** for any training file.
...
1. On `/training-files`, click **Download JSONL** for any training file.
...
1. On `/conversations`, select additional approved conversations.
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/api/training-files
```
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/api/datasets
```
...
**Entry points:** `/pipeline/configure` and `/pipeline/jobs`
...
1. Navigate to `/pipeline/configure`.
...
1. Navigate to `/pipeline/jobs`.
2. Observe: only your jobs are listed (scoped to `user_id = YOUR_UUID`).
3. Click the job to go to `/pipeline/jobs/{jobId}`.
...
1. On `/pipeline/jobs/{jobId}`, click **Cancel**.
...
1. On `/pipeline/jobs/{jobId}`, click **Deploy Adapter**.
...
1. Navigate to `/pipeline/jobs/{jobId}/test`.
...
1. On `/pipeline/jobs/{jobId}`, click **Download Adapter**.
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/api/pipeline/jobs
```
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  http://localhost:3000/api/pipeline/jobs/OTHER_USER_JOB_ID
```
...
1. On `/rag`, select your knowledge base.
...
1. Navigate to `/rag/{document_id}` and open the **Chat** tab.
...
2. Call: `GET /api/rag/query?knowledgeBaseId={kb_uuid}&limit=10`
...
2. Call: `POST /api/rag/quality` with `{ "queryId": "<uuid>" }`.
...
To verify that a signed-in user cannot see or modify another user's records, perform this test with **two separate browser sessions** (e.g. one normal window and one private/incognito window).
...
3. From **Session B**, navigate to: `http://localhost:3000/api/conversations/{id_from_session_A}`
4. **Observe:** `404 Not Found` — not `403 Forbidden`. The record appears not to exist.
5. From **Session B**, attempt: `PATCH /api/conversations/{id_from_session_A}` with `{ "status": "approved" }`
6. **Observe:** `404 Not Found`.
7. From **Session B**, call: `GET /api/conversations`
...
The identity spine removes all reliance on the `x-user-id` HTTP header. Verify this is the case.
...
All endpoints must return `401` when called without a session cookie.
...
for endpoint in \
  ...
  code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:3000${endpoint}")
  echo "${endpoint}: ${code}"
done
```
...
To exercise the entire platform in dependency order:
...
23. Run Part 6 identity spine verification queries
```
