...
- Database table: `lora_model_artifacts` (to store artifact records)
...
- Database table: `lora_datasets` (to link artifacts to datasets)
- Used for: Dataset name display in artifact records

#### Section E03: Training Configuration
- Database table: `lora_training_jobs` (to link artifacts to jobs)
...
- `lora_model_artifacts` - Created in E01 migration - Store artifact records
- `lora_notifications` - Created in E01 migration - Notify users when models are ready
- `lora_datasets` - Created in E01 migration - Link artifacts to source datasets
...
- `lora_datasets` - Get dataset name for artifact naming
...
#### Section E03: Training Configuration
**Database Tables We'll Query:**
- `lora_training_jobs` - Get job configuration and hyperparameters
...
#### Section E04: Training Execution & Monitoring
**Database Tables We'll Query:**
- `lora_training_jobs` - Find completed jobs without artifacts
...
- ✅ `lora_model_artifacts` table (from Section E01)
...
# - GPU_CLUSTER_API_URL: https://your-gpu-cluster.com/api
...
- ✅ `lora_model_artifacts` table with quality metrics
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .select(`
        *,
        dataset:lora_datasets!dataset_id(name),
        job:lora_training_jobs!job_id(preset_id, created_at)
      `, { count: 'exact' })
      .eq('user_id', user.id)
      .is('deleted_at', null);
...
export async function GET(
  ...
  try {
    ...
    const { data: model, error } = await supabase
      ...
      .select(`
        *,
        dataset:lora_datasets!dataset_id(id, name, format, total_training_pairs),
        job:lora_training_jobs!job_id(id, preset_id, created_at, started_at, completed_at)
      `)
      .eq('id', modelId)
      .eq('user_id', user.id)
      .is('deleted_at', null)
...
- [ ] Successfully queries `lora_training_jobs` for completed jobs
- [ ] Successfully queries `lora_metrics_points` for quality calculation
- [ ] Successfully queries `lora_datasets` for dataset info
- [ ] Successfully inserts into `lora_model_artifacts`
...
SELECT * FROM lora_model_artifacts 
WHERE job_id = '{job_id_from_above}';

-- Verify job was linked
SELECT artifact_id FROM lora_training_jobs 
...
**Expected Results:**
- Artifact record exists in `lora_model_artifacts`
...
**Database:**
- [ ] At least one artifact record in `lora_model_artifacts`
...
- [ ] Artifact records created in `lora_model_artifacts` table
...
- `lora_model_artifacts.training_summary.total_cost` - Final cost per model
