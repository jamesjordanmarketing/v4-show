 ...
 *  * Version 6.0 using prompt v6.0-v5
 * 
 ...
 * - pmc/product/_prompt_engineering/06a-product-task-elements-breakdown-prompt-v6.0-v5.md (template)
 * 
 * And outputs:
 * - pmc/product/_mapping/task-file-maps/6-[project-abbreviation]-tasks-E[XX].md (section files)
 * - pmc/product/_mapping/task-file-maps/prompts/06a-product-task-elements-breakdown-prompt-v6.0-E[XX].md (customized prompts)
...
function customizePrompt(promptTemplate, sectionHeader, sectionId, outputFilePath, projectAbbreviation) {
    ...
    // Replace [InputPromptFile] with the full absolute output file path
    customized = customized.replace(/\[InputPromptFile\]/g, outputFileFullPath);
    
    // Create OutputPromptFile by adding "output-" after "tasks-" in the filename
    const outputFileDir = path.dirname(outputFileFullPath);
    const outputFileName = path.basename(outputFileFullPath);
    const outputPromptFileName = outputFileName.replace(/tasks-/, 'tasks-output-');
    const outputPromptFileFullPath = path.join(outputFileDir, outputPromptFileName);
    
    // Replace [OutputPromptFile] with the modified output file path
    customized = customized.replace(/\[OutputPromptFile\]/g, outputPromptFileFullPath);
...
function generateIndexFile(sections, projectName, projectAbbreviation) {
    ...
    for (const sectionId of sectionIds) {
        ...
        indexContent += `- **Prompt File**: [Breakdown Prompt for ${sectionId}](./prompts/06a-product-task-elements-breakdown-prompt-v6.0-${sectionId}.md)\n\n`;
    }
...
function generateTaskSegments(projectName, projectAbbreviation) {
    ...
    for (const [sectionId, sectionContent] of Object.entries(sections)) {
        ...
        const promptFilePath = path.join(promptsOutputDir, `06a-product-task-elements-breakdown-prompt-v6.0-${sectionId}.md`);
    ...
    console.log(`Successfully generated all files using prompt template v6.0-v5.`);
}
...
if (require.main === module) {
    ...
    if (args.length !== 2) {
        console.error('Usage: node <path_to_script>/06b-generate-task-prompt-segments_v6.0.js <"Project Name"> <project-abbreviation>');
        console.error('Examples:');
        console.error('  From project root:');
        console.error('  node product/_tools/06b-generate-task-prompt-segments_v6.0.js "Bright Run LoRA Training Product" bmo');
        console.error('');
        console.error('  From any directory:');
        console.error('  node /path/to/pmc/product/_tools/06b-generate-task-prompt-segments_v6.0.js "Bright Run LoRA Training Product" bmo');
        process.exit(1);
    }
