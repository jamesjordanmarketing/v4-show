/**
 * Test Mapping Files Concatenation Script
 * 
 * This script concatenates test mapping fragment files from a source directory
 * into a single consolidated file based on a template.
 * 
 * The script reads from:
 * - pmc/product/_mapping/test-maps/06-[project-abbreviation]-task-test-mapping-E[##].md (fragment files)
 * - pmc/product/_templates/07-tasks-tests-built-template.md.md (template file)
 * 
 * And outputs:
 * - pmc/product/07b-task-[project-abbreviation]-testing-built.md (consolidated file)
 */
...
function extractProjectInfo() {
    try {
        const overviewPath = resolveProjectPath('product/01-aplio-mod-1-overview.md');
        ...
        const abbreviation = abbreviationMatch ? abbreviationMatch[1] : 'aplio-mod-1';
...
/**
 * Extract numerical suffix from filename (e.g., 'E01' from '06-aplio-mod-1-task-test-mapping-E01.md')
 ...
 * @param {string} filePrefix - File prefix pattern to match (e.g., '06-aplio-mod-1-task-test-mapping-')
...
function processTemplate(templatePath, variables, sourceFileList, sourceDir) {
    try {
        ...
        const sourceListPlaceholder = '[list of 06-aplio-mod-1-task-test-mapping-E[##].md files that were concatenated]';
...
/**
 * Generate consolidated test mapping file
 * @param {Object} options - Options for file generation
 */
export async function generateConsolidatedTestMappingFile(options = {}) {
    ...
    if (sourceDirOverride) {
        ...
        const sourceDirRelative = 'product/_mapping/test-maps';
        sourceDir = findExistingPath(projectRoot, sourceDirRelative);
        
        if (!sourceDir) {
            console.error(`Could not find source directory for test mapping files.`);
            return;
        }
    }
    ...
    if (templatePathOverride) {
        ...
        const templateRelative = 'product/_templates/07-tasks-tests-built-template.md.md';
    ...
    const outputFileName = `07b-task-${projectAbbreviation.trim()}-testing-built.md`;
    const outputPath = outputPathOverride || path.join(outputDir, outputFileName);
    
    console.log(`Starting test mapping file concatenation...`);
    ...
    const filePrefix = `06-${projectAbbreviation}-task-test-mapping-`;
    ...
    try {
        ...
        console.log(`Successfully generated consolidated test mapping file: ${outputPath}`);
    } catch (error) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    generateConsolidatedTestMappingFile();
}

export default { generateConsolidatedTestMappingFile }; 