...
function getCommandLineArguments() {
  ...
  if (args.length !== 2) {
    ...
    console.error('Usage: node 07-generate-task-test-mapping_v4.js "<Project Name>" <product-abbr>');
    console.error('Example: node 07-generate-task-test-mapping_v4.js "Document Categorization Module" bmo');
    process.exit(1);
  }
  return {
    productName: args[0],
...
function createCustomPromptFile(sectionId, paddedSectionId, promptsDir, sectionTasks, productAbbr, projectInfo) {
  try {
    ...
    // Create input and output file paths for this section
    const baseProjectDir = getBaseProjectDir();
    const inputFileName = `6-${productAbbr}-tasks-E${paddedSectionId}.md`;
    const outputFileName = `07-${productAbbr}-test-mapping-output-E${paddedSectionId}.md`;
    
    // Create full paths for input and output files
    const inputFilePath = path.join(baseProjectDir, 'pmc', 'product', '_mapping', 'task-file-maps', inputFileName);
    const outputFilePath = path.join(baseProjectDir, 'pmc', 'product', '_mapping', 'test-maps', outputFileName);
    
    // Replace [OutputPromptFile] placeholder with the output file path
    ...
    console.log(`Output file path set to: ${outputFilePath}`);
  } catch (error) {
