...
function customizePrompt(promptTemplate, sectionHeader, sectionId, outputFilePath, projectAbbreviation) {
    ...
    // NEW: Set [Task Scope Reference File For This Prompt] to the original output file path
    customized = customized.replace(/\[Task Scope Reference File For This Prompt\]/g, `\`${outputFileFullPath}\``);
    
    // NEW: Create the modified output file path with -output suffix
    const outputFileWithSuffix = outputFileFullPath.replace(/\.md$/, '-output.md');
    
    // Replace [Output File For This Prompt] with the modified output file path (with -output suffix)
    // Note: Case insensitive replace to catch both variants (Prompt vs prompt)
    customized = customized.replace(/\[Output File For This [Pp]rompt\]/g, `\`${outputFileWithSuffix}\``);
...
function generateIndexFile(sections, projectName, projectAbbreviation) {
    ...
    for (const sectionId of sectionIds) {
        ...
        indexContent += `- **Prompt File**: [Breakdown Prompt for ${sectionId}](./prompts/06a-product-task-elements-breakdown-prompt-v5.4-${sectionId}.md)\n\n`;
    }
...
if (require.main === module) {
    ...
    if (args.length !== 2) {
        console.error('Usage: node <path_to_script>/06b-generate-task-prompt-segments-new.js <"Project Name"> <project-abbreviation>');
        console.error('Examples:');
        console.error('  From project root:');
        console.error('  node product/_tools/06b-generate-task-prompt-segments-new.js "Aplio Design System Modernization" aplio-mod-1');
        console.error('');
        console.error('  From any directory:');
        console.error('  node /path/to/pmc/product/_tools/06b-generate-task-prompt-segments-new.js "Aplio Design System Modernization" aplio-mod-1');
        process.exit(1);
    }
