...
/**
 * Gets the product abbreviation from command line arguments
 * @returns {string} The product abbreviation
 */
function getProductAbbreviation() {
  const args = process.argv.slice(2);
  if (args.length === 0) {
    console.error('Error: Product abbreviation is required as a command line argument.');
    console.error('Usage: node 07-generate-task-test-mapping_v2.js <product-abbr>');
    console.error('Example: node 07-generate-task-test-mapping_v2.js aplio-mod-1');
    process.exit(1);
  }
  return args[0];
}
...
function generateTaskTestMapping() {
  const productAbbr = getProductAbbreviation();
  console.log(`Generating Task-to-Test Mapping documents for product: ${productAbbr}...`);
  ...
  if (taskFiles.length === 0) {
    ...
    processLegacyTaskFile(baseOutputDir, testMapsDir, promptsDir, productAbbr);
    return;
  }
  ...
  taskFiles.forEach(taskFile => {
    ...
    const mappingDocument = generateMappingStructure(tasks, sectionId, sectionName, productAbbr);
    ...
    createCustomPromptFile(sectionId, paddedSectionId, promptsDir, tasks, productAbbr);
  ...
  if (allTasks.length > 0) {
    const consolidatedDocument = generateConsolidatedDocument(allTasks, productAbbr);
    ...
    const consolidatedIndex = generateConsolidatedIndex(taskGroups, productAbbr);
...
function processLegacyTaskFile(baseOutputDir, testMapsDir, promptsDir, productAbbr) {
  ...
  try {
    ...
    const consolidatedDocument = generateConsolidatedDocument(tasks, productAbbr);
    const documentWithPlaceholders = addHumanInputPlaceholders(consolidatedDocument);
    ...
    Object.keys(taskGroups).sort().forEach(sectionId => {
      ...
      // Generate the mapping document for this section
      const mappingDocument = generateMappingStructure(sectionTasks, sectionId, sectionName, productAbbr);
      ...
      // Create a customized prompt file for this section
      createCustomPromptFile(sectionId, paddedSectionId, promptsDir, sectionTasks, productAbbr);
...
function createCustomPromptFile(sectionId, paddedSectionId, promptsDir, sectionTasks, productAbbr) {
  try {
...
function generateConsolidatedDocument(tasks, productAbbr) {
  let output = '';
  
  // Document header with dynamic product name
  const productName = getProductDisplayName(productAbbr);
  output += `# ${productName} - Task to Test Mapping\n`;
...
function generateMappingStructure(tasks, sectionId, sectionName, productAbbr) {
  let output = '';
  
  // Document header with dynamic product name
  const productName = getProductDisplayName(productAbbr);
  output += `# ${productName} - Task to Test Mapping - Section ${sectionId}\n`;
  output += `**Generated:** ${new Date().toISOString().split('T')[0]}\n\n`;
...
function generateConsolidatedIndex(taskGroups, productAbbr) {
  let output = '';
...
// Helper function to get product display name from abbreviation
function getProductDisplayName(productAbbr) {
  // Common product abbreviation to display name mappings
  const productNames = {
    'aplio-mod-1': 'Aplio Design System Next.js Modernization',
    'brightgrid-mod-1': 'BrightGrid Design System Next.js Modernization',
    'coralui-mod-1': 'CoralUI Design System Next.js Modernization'
  };
  
  return productNames[productAbbr] || `${productAbbr} Design System Next.js Modernization`;
}
