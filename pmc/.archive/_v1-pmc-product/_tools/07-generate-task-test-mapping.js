...
function generateTaskTestMapping() {
  console.log('Generating Task-to-Test Mapping documents...');
  ...
  if (taskFiles.length === 0) {
    ...
    processLegacyTaskFile(baseOutputDir, testMapsDir, promptsDir);
    return;
  }
  ...
  taskFiles.forEach(taskFile => {
    ...
    const mappingDocument = generateMappingStructure(tasks, sectionId, sectionName);
    
    // Add placeholders for human input
    const documentWithPlaceholders = addHumanInputPlaceholders(mappingDocument);
    
    // Write the output markdown file to the test-maps directory
    const mdOutputPath = path.join(testMapsDir, `06-aplio-mod-1-task-test-mapping-E${paddedSectionId}.md`);
  ...
  if (allTasks.length > 0) {
    const consolidatedDocument = generateConsolidatedDocument(allTasks);
    ...
    const consolidatedIndex = generateConsolidatedIndex(taskGroups);
    const indexPath = path.join(testMapsDir, '06-aplio-mod-1-task-test-mapping-index.md');
...
function processLegacyTaskFile(baseOutputDir, testMapsDir, promptsDir) {
  console.log('Attempting to process legacy task file...');
  
  try {
    // Try to read the legacy tasks file
    const legacyFilePath = path.join(process.cwd(), 'product', '06-aplio-mod-1-tasks.md');
    ...
    const consolidatedDocument = generateConsolidatedDocument(tasks);
    const documentWithPlaceholders = addHumanInputPlaceholders(consolidatedDocument);
    ...
    Object.keys(taskGroups).sort().forEach(sectionId => {
      ...
      // Generate the mapping document for this section
      const mappingDocument = generateMappingStructure(sectionTasks, sectionId, sectionName);
      ...
      // Create a customized prompt file for this section
      createCustomPromptFile(sectionId, paddedSectionId, promptsDir, sectionTasks);
...
function generateConsolidatedDocument(tasks) {
  let output = '';
  
  // Document header
  output += '# Aplio Design System Next.js Modernization - Task to Test Mapping\n';
...
function generateMappingStructure(tasks, sectionId, sectionName) {
  let output = '';
  
  // Document header
  output += `# Aplio Design System Next.js Modernization - Task to Test Mapping - Section ${sectionId}\n`;
  output += `**Generated:** ${new Date().toISOString().split('T')[0]}\n\n`;
...
function generateConsolidatedIndex(taskGroups) {
  let output = '';
  
  // Document header
  output += '# Aplio Design System Next.js Modernization - Task to Test Mapping Index\n';
  output += `**Generated:** ${new Date().toISOString().split('T')[0]}\n\n`;
  ...
  // List all section files with links
  Object.keys(taskGroups).sort().forEach(sectionId => {
    const sectionName = getSectionName(sectionId, [].concat(...Object.values(taskGroups)));
    const paddedSectionId = sectionId.padStart(2, '0');
    output += `- [Section ${sectionId}: ${sectionName}](./06-aplio-mod-1-task-test-mapping-E${paddedSectionId}.md)\n`;
  });
